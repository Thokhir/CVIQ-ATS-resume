...
from utils.database import (
    ...
    grant_temporary_full_access, has_temp_access, revoke_temp_access,
...
def show_admin_panel():
    ...
    st.caption("Give anyone complete access (all tools, unlimited optimizations) for a limited window — "
               "without touching their plan or credits. They must already have an account. "
        ...
        for g in active_grants:
            ...
            with gc1:
                st.markdown(
                    ...
                    f'👤 <b>{g["username"]}</b> ({g["email"]}) — ⏳ until <b style="color:#3ecf8e">{g["temp_access_until"]}</b></div>',
                    unsafe_allow_html=True)
            with gc2:
                if st.button("Revoke", key=f"ta_rev_{g['id']}", use_container_width=True):
                    revoke_temp_access(g["id"])
                    st.warning(f"Revoked access for {g['username']}")
                    st.rerun()
